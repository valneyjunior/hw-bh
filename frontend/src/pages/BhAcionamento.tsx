...
import { getDisponiveis } from '../services/api'
import type { Disponivel } from '../types/bh'
...
// Normaliza telefone BR para link do WhatsApp (wa.me espera só dígitos com DDI).
...
function linkTel(telefone: string): string {
  return `tel:${telefone.replace(/\s/g, '')}`
}
...
export default function BhAcionamento() {
  const hoje = new Date()
  const [ano, setAno] = useState(hoje.getFullYear())
  const [mes, setMes] = useState(hoje.getMonth())
  const [escala, setEscala] = useState<Disponivel[]>([])
  ...
  const [telRevelado, setTelRevelado] = useState<Set<number>>(new Set())
  ...
  async function carregar(a: number, m: number) {
    ...
    try {
      ...
      const res = await getDisponiveis({ from, to })
      setEscala(res.data as Disponivel[])
    } catch {
      setEscala([])
    } finally {
      setCarregando(false)
  ...
  function navegarMes(delta: number) {
    ...
    setAno(novoAno)
    setDiaDetalhe(null)
    setTelRevelado(new Set())
  }

  function toggleTel(id: number) {
    setTelRevelado((prev) => {
  ...
  // Setores únicos presentes neste mês
  ...
  const escalaByData = useMemo(() => {
    const map = new Map<string, Disponivel[]>()
  ...
  function exportarCSV() {
    const linhas = [['Data', 'Dia da Semana', 'Colaborador', 'Setor', 'Turno', 'Telefone']]
    ...
    a.href = url
    a.download = `acionamento-${NOMES_MES[mes].toLowerCase()}-${ano}.csv`
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Acionamento de Voluntários</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="grid grid-cols-7 gap-1">
                {dias.map((cell, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-xs font-medium mb-1 ${ehHoje ? 'text-red-600' : 'text-gray-500'}`}>
                        {cell.dia}
                      </span>
        ...
        <div className="space-y-4">
          {/* Detalhe do dia — com acionamento */}
          {diaDetalhe && (
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              {detalheDia.length === 0 ? (
                ...
                <div className="space-y-3">
                  {detalheDia.map((e) => {
                    const revelado = telRevelado.has(e.id)
                    const wpp = e.telefone ? linkWhatsapp(e.telefone) : null
                    return (
                      <div key={e.id} className="border border-gray-100 rounded-lg p-2.5">
                        ...
                        {!revelado ? (
                          <button
                            onClick={() => toggleTel(e.id)}
                            className="mt-2.5 w-full text-xs font-medium px-2 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors"
                          >
                            Visualizar número de telefone
                          </button>
                        ) : e.telefone ? (
                          <div className="mt-2.5 space-y-2">
                            <p className="text-sm font-semibold text-gray-800 text-center tracking-wide">{e.telefone}</p>
                            <div className="flex gap-2">
                              <a href={linkTel(e.telefone)}
                                className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium px-2 py-1.5 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50">
                                📞 Ligar
                              </a>
                              {wpp && (
                                <a href={wpp} target="_blank" rel="noopener noreferrer"
                                  className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium px-2 py-1.5 rounded-lg text-white hover:opacity-90"
                          ...
                          <p className="mt-2.5 text-xs text-amber-600 text-center bg-amber-50 border border-amber-100 rounded-lg py-1.5">
